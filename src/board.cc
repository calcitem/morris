...
bool Move::operator==(const Move &m) const
{
    ...
    if (newPos != m.newPos)
        return false;
    if (mode == Mode_Move && oldPos != m.oldPos)
        return false;
    if (mode != m.mode)
        return false;

    if (takes.size() != m.takes.size())
        return false;
...
bool Board::operator==(const Board &b) const
{
    ...
    if (currentPlayer != b.currentPlayer)
        return false;

    for (int i = 0; i < 2; i++) {
        if (nPiecesToSet[i] != b.nPiecesToSet[i])
            return false;
        if (nPiecesOnBoard[i] != b.nPiecesOnBoard[i])
            return false;
    }

    for (int i = 0; i < MAXPOSITIONS; i++) {
        if (boardPos[i] != b.boardPos[i])
            return false;
    }
