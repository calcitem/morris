    ...
    if (options.animateComputerMoves) {
        ...
        for (int i = 0; i < m.takes.size(); i++) {
            ...
            if (take_moveOut) {
                ...
                anim->endPos.x = ((anim->player == PL_White) ? geom.windowWidth_inUnits + geom.piece_radius_inUnits : -geom.piece_radius_inUnits);
...
Point2D BoardGUI_GtkCairo::Geometry::getStackPiecePos(Player p, int n) const
{
    ...
    if (p == PL_White)
        pt.x = stackX_white_inUnits;
    else
        pt.x = stackX_black_inUnits;
    ...
    if (boardGrid) {
        ...
        for (int b = 0; b < 2; b++)
            for (int i = 0; i < coords.size(); i++) {
                ...
                p.x = coords[i];
                if (b == 0)
                    p.y = 1.0 + 0 * geom.boardBorderBottom_inUnits / 2;
                else
                    p.y = 0.0 - 0 * geom.boardBorderTop_inUnits / 2;
        ...
        for (int b = 0; b < 2; b++)
            for (int i = 0; i < coords.size(); i++) {
                ...
                Point2D p;
                if (b == 0)
                    p.x = 1.0 + geom.boardBorderRight_inUnits / 2;
                else
                    p.x = -geom.boardBorderLeft_inUnits / 2;
                p.y = coords[i];
                p = geom.board2Pixel(p);
...
void BoardGUI_GtkCairo::draw_board(int x0, int y0, int w, int h)
{
    ...
    if (drag_active) {
        draw_piece_inPixels(cr, Point2D(drag_position.x + drag_offset_x, drag_position.y + drag_offset_y), drag_player);
    }
...
void BoardGUI_GtkCairo::startPieceDrag(Position p, Point2D pixelPos)
{
    ...
    if (options.coloredCrossingWhileDragging)
        redrawBoard();
}
...
void BoardGUI_GtkCairo::Geometry::fitInto(int w, int h, const BoardGUI_GtkCairo::Options &options)
{
    ...
    float coordBorder = 0;
    if (options.showCoordinates)
        coordBorder = piece_radius_inUnits * 0.8;
