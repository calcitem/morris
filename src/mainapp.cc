...
void MainApp::startNextMove()
{
    ...
    */
    ...
    public:
        void operator()()
        {
            MainApp::app().getControl().startNextMove();
        }
    };
...
void MainApp::setPause(bool flag)
{
    ...
    if (changed)
        m_signal_pauseChanged(appState.gamePaused);
}
...
{
    if (!p->isInteractivePlayer())
        gui_application->getBoardGUI()->startNonInteractiveMove();
}

void MainApp::endMove(player_ptr p)
{
    if (!p->isInteractivePlayer())
        gui_application->getBoardGUI()->endNonInteractiveMove();
}
