...
bool RuleSpec::isValidMove(const Board &b, const Move &m) const
{
    ...
    case Move::Mode_Move:
        if (b.getPosition(m.newPos) != PL_None) {
            return false;
        }
        if (b.getPosition(m.oldPos) != b.getCurrentPlayer()) {
            return false;
        }
...
bool RuleSpec::isGameOver(const Board &b, Player *winner) const
{
    ...
    if (currentPlayerHasLost(b)) {
        if (winner)
            *winner = opponent(b.getCurrentPlayer());
        return true;
    }
...
{
    BoardID id = 0;
    id += board.getNPiecesToSet(PL_White);
    id *= nPieces + 1;
